...
it("getEngineByFilePath should return default engine for file with no extension", () => {
	...
	expect(ecto.getEngineByFilePath("/path/to/file-without-extension")).toBe(
		"ejs",
	);
});
...
it("renderSync should handle errors and return empty string", () => {
	...
	const malformedEjsSource =
		"<% if (test) { %><h2><%= test.foo %></h2><% } %><%";
...
it("render via handlebars and not define engineName", async () => {
	...
	expect(
		await ecto.render(handlebarsExampleSource, handlebarsExampleData),
	).toBe(handlebarsExampleSource);
});
...
it("Render from Template - EJS", async () => {
	...
	const source = await ecto.renderFromFile(
		`${testRootDirectory}/ejs/example1.ejs`,
		ejsExampleData2,
		`${testRootDirectory}/ejs`,
	);
...
it("Render from Template - Default to EJS", async () => {
	const ecto = new Ecto();
	const source = await ecto.renderFromFile(
		`${testRootDirectory}/ejs/example1.html`,
		ejsExampleData2,
		`${testRootDirectory}/ejs`,
	);
...
it("Render with Configuration via Nunjucks", async () => {
	...
	const ecto = new Ecto({
		...
		engineOptions: { nunjucks: nunjucksOptions },
	});
	const userInput = "<script>alert('XSS')</script>";
	const source = await ecto.renderFromFile(
		`${testRootDirectory}/nunjucks/example2.njk`,
		{ name: userInput },
	);
