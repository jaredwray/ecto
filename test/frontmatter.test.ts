...
describe("Ecto FrontMatter", async () => {
	const frontMatterDocument = await fs.promises.readFile(
		"./test/data/markdown/frontmatter.md",
		"utf8",
	);
	const noFrontMatterDocument = await fs.promises.readFile(
		"./test/data/markdown/no-frontmatter.md",
		"utf8",
	);
	...
	test("should add / update front matter with already existing", async () => {
		...
		expect(updatedFrontMatterContent?.tags).toEqual([
			"project",
			"documentation",
		]);
	});
