...
const testTemplateDir = "./testing/mustache";
...
test("Mustache - Render with Partials", async () => {
    let engine = new Mustache();
    engine.rootTemplatePath = testTemplateDir;
    let source = await fs.readFile(testTemplateDir + "/example1.mustache", "utf8");
    let output = await engine.render(source, exampleData2)

    expect(output).toContain("Joe spends 6");
